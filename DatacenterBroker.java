...
//	}
	...
	protected void submitCloudlets()
	{
		int solutioncount=20,temp;
		int avg=(getCloudletList().size()/getVmsCreatedList().size())+1;
		int []cloudletCount=new int[getVmsCreatedList().size()];
		//fitness of solutions in population
		double [][] fitness=new double[solutioncount][2];
		//exec time for each vm of a solution
	  double [] vmexectime=new double[getVmsCreatedList().size()];
		//list to store initial population
		List<ArrayList<ArrayList<Integer>>> Population=new ArrayList<ArrayList<ArrayList<Integer>>>();
		//list to store best solution
		List<ArrayList<Integer>> bestSoln=new ArrayList<ArrayList<Integer>>();
		...
		//to generate fitness of each solution
		 for(int i=0;i<solutioncount;i++)
		 {
			 ...
			 for(int j=0;j<getVmsCreatedList().size();j++)
			 {
				 ...
				 for(int k=0;k<Population.get(i).get(j).size();k++)
				 {
					 ...
					 totalcloudletlength+=CloudletList.getById(getCloudletList(),Population.get(i).get(j).get(k)).getCloudletLength();
				 }

				 //calculate time duration for each vm
				 double duration=(double)totalcloudletlength/(double)vmmips;
				 vmexectime[j]=duration;
				 System.out.println(vmexectime[j]);
			 }
			 System.out.println("\n");
			 fitness[i][0]=i;

			 //find max time duration(makespan) for a solution and store it.
			 fitness[i][1]=max(vmexectime);
		 }
		 //print fitness
		 for(int i=0;i<fitness.length;i++)
		 {
			 for(int j=0;j<2;j++)
			 {
				 System.out.println(fitness[i][j]);
			 }
		 }

		 //find best solution no
		 int bestsolutionno=bestSol(fitness);
		 bestSoln.addAll(Population.get(bestsolutionno));

		 System.out.println(bestsolutionno);
		 ...
		 for(int i=0;i<solutioncount;i++)
		 {
		     ...
		     if(rand<=0.8)
		     {
					  //generating the pivot point
						//global pollination
						int pivot=(int)(Math.random()*(getVmsCreatedList().size()));
						for(int j=pivot;j<=getVmsCreatedList().size();j++)
						{
							temp = bestSolnPopulation.get(bestsolutionno).get(j);
							Population.get(i).set(j,temp);
						}
		     }
		     else
		     {
		    	 ...
		    	 //local pollination
					 ...
					 for(int j=pivot;j<=getVmsCreatedList().size();j++)
					 {
						 int randomSoln=(int)(Math.random()*solutioncount);
						 temp = Population.get(randomSoln).get(j);
						 Population.get(i).set(j,temp);
					 }
				}
		     }
	...
	//to sort the fitnesses and find best solution no.
	protected int bestSol(double[][] arr)
	{
		int rows=arr.length;
